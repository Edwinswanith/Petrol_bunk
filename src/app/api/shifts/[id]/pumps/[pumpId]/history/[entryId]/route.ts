...
import { pumpShiftCorrectionSchema, pumpShiftVoidSchema } from "@/server/http/schemas";
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string; pumpId: string; entryId: string }> }) {
  try {
    const { id, pumpId, entryId } = await context.params;
    const input = pumpShiftVoidSchema.parse(await request.json());
    const shift = await getOperationsRepository().voidPumpShiftEntry(id, pumpId, entryId, input);
    return NextResponse.json(shift);
  } catch (error) {
    return apiError(error);
  }
