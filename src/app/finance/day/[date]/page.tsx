...
export default async function FinanceDayPage({ params }: { params: Promise<{ date: string }> }) {
  ...
  return <main className="page finance-page">
    ...
    <section className="finance-scoreboard"><article><span>Closed shifts</span><strong>{day?.shifts ?? 0}</strong></article><article><span>Overall sales</span><strong>{money(day?.revenue ?? "0")}</strong></article><article><span>Fuel gross profit</span><strong>{money(day?.grossMargin ?? "0")}</strong></article><article><span>Operating expenses</span><strong>{money(day?.expenses ?? "0")}</strong></article></section>
