...
export function StaffRegister({ staff, resignedStaff = [], attendance, payroll, date, month }: { staff: StaffRecord[]; resignedStaff?: StaffRecord[]; attendance: AttendanceRecord[]; payroll: PayrollRecord[]; date: string; month: string }) {
  ...
  function updateStaffDetails(event: FormEvent<HTMLFormElement>, staffId: string) {
    event.preventDefault(); const form = new FormData(event.currentTarget);
    void post(`/api/staff/${staffId}`, { name: String(form.get("name") || ""), phone: String(form.get("phone") || ""), monthlySalary: String(form.get("monthlySalary") || "0"), dailyBeta: String(form.get("dailyBeta") || "0"), assignedShift: String(form.get("assignedShift") || "SHIFT_1") }, "PATCH");
  }
  ...
  return <div className="staff-register-grid">
    ...
    <section className="panel panel-pad salary-panel">
      ...
      {staff.length ? <div className="salary-list">{staff.map((person) => <div className="salary-row" key={person.id}><form onSubmit={(event) => updateStaffDetails(event, person.id)}><label><span>Name</span><input aria-label={`${person.name} name`} defaultValue={person.name} minLength={2} name="name" required /></label><label><span>Phone</span><input aria-label={`${person.name} phone`} defaultValue={person.phone ?? ""} inputMode="tel" name="phone" placeholder="98765 43210" /></label><label><span>Shift</span><select aria-label={`${person.name} assigned shift`} defaultValue={person.assignedShift ?? "SHIFT_1"} name="assignedShift"><option value="SHIFT_1">Shift 1</option><option value="SHIFT_2">Shift 2</option></select></label><label><span>Monthly salary</span><span className="salary-input"><b>₹</b><input aria-label={`${person.name} monthly salary`} defaultValue={person.monthlySalary ?? "0"} min="0" name="monthlySalary" required step="0.01" type="number" /></span></label><label><span>Daily beta</span><span className="salary-input"><b>₹</b><input aria-label={`${person.name} daily beta`} defaultValue={person.dailyBeta ?? "0"} min="0" name="dailyBeta" required step="0.01" type="number" /></span></label><button aria-label={`Save ${person.name} details`} className="icon-button" disabled={saving} type="submit"><Save size={15} /></button></form><div className="salary-row-actions"><ResignStaffButton onDone={() => router.refresh()} person={person} /></div></div>)}</div> : <p className="empty-state">Add a staff member to configure salary.</p>}
    </section>
    ...
    <section className="panel panel-pad payroll-panel">
      ...
      {payroll.length ? <details className="payroll-history"><summary>View payroll history</summary><DataTable><thead><tr><th>Month</th><th>Staff</th><th>Net</th><th>Paid</th><th>Balance</th></tr></thead><tbody>{payroll.map((record) => <tr key={record.id}><td>{record.month}</td><td>{record.staffName}</td><td>₹{record.netPay}</td><td>₹{record.amountPaid}</td><td>₹{record.balanceDue}</td></tr>)}</tbody></DataTable></details> : null}
    </section>
  </div>;
}
