...
export function DailyForecourtSheet({ businessDate, today = businessDate, products, staff, stations, tanks, tankLevels = [], missingBusinessDays = [], previousReadings, previousReadingSources = {}, activeShift, attendance }: Props) {
  ...
  const recordingDate = activeShift ? confirmedBusinessDate || activeShift.businessDate : "";
  const activeDateWarning = !activeShift ? ""
    : activeBusinessDateDraft > today ? `${activeBusinessDateDraft} is in the future. Choose today (${today}) or an earlier day.`
    : recordingDate > today ? `Recording date ${recordingDate} is in the future. Change it to today (${today}) before saving more entries.`
    : recordingDate !== today ? `Entries will be saved under ${recordingDate}, not today (${today}).`
    : "";
  ...
  return <div className="daily-sheet">
    ...
    {!activeShift ? <><section className="today-setup-strip"><label className="field"><span>Business date</span><input form="daily-opening-form" value={businessDateDraft} onChange={(event) => setBusinessDateDraft(event.target.value)} max={today} name="businessDate" type="date" required />{businessDateDraft > today ? <small className="recording-date-warning" role="alert">This date is in the future. Choose today ({today}) or an earlier day.</small> : null}</label><form className="inline-operator-form" onSubmit={addOperator}><label><span>Add operator without leaving Today</span><input name="name" placeholder="Operator name" required /></label><button className="button soft" disabled={saving}><Plus size={14} />Add</button></form><div className="attendance-chips">{attendance.map((record) => <span className={`attendance-chip ${record.status.toLowerCase()}`} key={record.staffId}>{record.staffName} · {record.status}</span>)}</div></section><form id="daily-opening-form" onSubmit={openDay}>
