...
export function DailyForecourtSheet({ businessDate, products, staff, stations, tanks, previousReadings, previousReadingSources = {}, activeShift, attendance }: Props) {
  ...
  useEffect(() => {
    ...
    writeDraft(closingDraftKey, { operatorIds, openingReadings, closingReadings, collections, testFuel, testFuelReturned, closingTankStocks, rates, activeCorrectionReason, varianceExplanation } satisfies ClosingDraft);
    // eslint-disable-next-line react-hooks/set-state-in-effect -- reflects the write we just made to localStorage, an external system
    setDraftSavedAt(new Date());
  }, [activeShift, closingDraftKey, operatorIds, openingReadings, closingReadings, collections, testFuel, testFuelReturned, closingTankStocks, rates, activeCorrectionReason, varianceExplanation]);
