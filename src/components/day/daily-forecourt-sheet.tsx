...
import { Calculator, CheckCircle2, Fuel, Gauge, IndianRupee, LockKeyhole, PencilLine, Play, UserRound } from "lucide-react";
  ...
  return <div className="daily-sheet">
    ...
    {!activeShift ? <form onSubmit={openDay}>
      <section className="daily-rate-board"><header><span><small>Step 1 · Set today&apos;s rates</small><strong>Dealer cost &amp; customer price</strong></span><p>These values are locked into today&apos;s sales record and will not change historical profit.</p></header><div className="daily-price-deck">
        {products.map((product) => <article className={`price-ticket ${product.id}`} key={product.id}><div className="price-product"><span className="fuel-dot" /><span><small>Fuel grade</small><strong>{product.name}</strong></span></div><label className="rate-field"><span><small>What the outlet pays</small><strong>Reseller purchase price</strong></span><span className="money-control"><b>₹</b><input aria-label={`${product.name} reseller purchase price`} defaultValue={product.costPricePerLitre} min="0" name={`cost-${product.id}`} required step="0.01" type="number" /><em>per litre</em></span></label><label className="rate-field customer"><span><small>What the customer pays</small><strong>Customer selling price</strong></span><span className="money-control"><b>₹</b><input aria-label={`${product.name} customer selling price`} defaultValue={product.sellingPricePerLitre} min="0" name={`selling-${product.id}`} required step="0.01" type="number" /><em>per litre</em></span></label></article>)}
      </div></section>
      ...
      <section className="daily-price-deck compact">{products.map((product) => { const station = stations.find((item) => item.productId === product.id); return <article className={`price-ticket ${product.id}`} key={product.id}><div className="price-product"><span className="fuel-dot" /><span><small>Locked daily rates</small><strong>{product.name}</strong></span></div><span className="locked-rate"><small>Reseller</small><strong>₹{station?.costPerLitre ?? product.costPricePerLitre}</strong></span><span className="locked-rate customer"><small>Customer</small><strong>₹{station?.pricePerLitre ?? product.sellingPricePerLitre}</strong></span></article>; })}</section>
...
function PumpDeck({ pumps, staff, previousReadings }: { pumps: Pump[]; staff: Staff[]; previousReadings: Record<string, string> }) {
  return <><div className="section-step"><span>2</span><div><small>Staff &amp; meter setup</small><strong>Assign each side, then enter every opening totalizer independently</strong></div></div><section className="pump-deck">{pumps.map((pump) => <article className="pump-card" key={pump.code}><header><span className="pump-emblem"><Fuel size={20} /></span><span><small>Multi-product dispenser</small><strong>Pump {pump.code}</strong></span><Gauge size={22} /></header><div className="pump-sides">{pump.sides.map((side) => <section className="pump-side" key={side.id}><div className="side-owner"><span><small>{side.label}</small><strong>{side.stations.map((station) => station.code).join(" + ")}</strong></span><label className="operator-picker"><span><UserRound size={14} /><small>Operator for this side</small></span><select aria-label={`Pump ${pump.code} ${side.label} operator`} name={`staff-${side.id}`} required><option value="">Select staff member</option>{staff.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label></div><div className="nozzle-list">{side.stations.map((station) => <div className="nozzle-entry" key={station.stationId}><div className="nozzle-identity"><span className={`nozzle-badge ${station.productId}`}><Fuel size={14} />N{station.nozzleNumber ?? station.code}</span><span><strong>{station.productName}</strong><small>{previousReadings[station.stationId] ? `Last closing: ${previousReadings[station.stationId]}` : "No previous reading"}</small></span></div><label className="totalizer-field"><span><PencilLine size={13} />Opening totalizer</span><span className="totalizer-control"><input aria-label={`${station.code} opening totalizer`} defaultValue={previousReadings[station.stationId] ?? ""} min="0" name={`opening-${station.stationId}`} placeholder="Enter reading" required step="0.001" type="number" /><em>litres</em></span></label></div>)}</div></section>)}</div></article>)}</section></>;
}
