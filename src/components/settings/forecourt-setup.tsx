...
export function ForecourtSetup({ configuration }: { configuration: ForecourtConfiguration }) {
  ...
  const updatePrice = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/products/${productId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sellingPricePerLitre: String(form.get("sellingPricePerLitre")), costPricePerLitre: String(form.get("costPricePerLitre")) }) });
  ...
  return <>
    ...
    <div className="three-column reveal reveal-2" aria-busy={saving}>
      <SetupForm title="Add fuel product" description="Petrol, diesel, XP95, X100 or any custom grade." icon={<Fuel color="#0d6b5d" size={20} />} button="Add product" disabled={saving} onSubmit={submit("/api/products")}>
        <label className="field"><span>Code</span><input name="code" placeholder="XP95" required /></label><label className="field"><span>Name</span><input name="name" placeholder="XP95" required /></label><label className="field"><span>Selling price / L</span><input min="0" name="sellingPricePerLitre" required step="0.01" type="number" /></label><label className="field"><span>Cost price / L</span><input min="0" name="costPricePerLitre" required step="0.01" type="number" /></label>
      </SetupForm>
      <SetupForm title="Update fuel price" description="The new price applies to future shifts; open and closed shifts retain their snapshots." icon={<Fuel color="#0d6b5d" size={20} />} button="Update price" disabled={saving} onSubmit={updatePrice}>
        <label className="field full"><span>Fuel product</span><select name="productId" required>{configuration.products.map((product) => <option key={product.id} value={product.id}>{product.name} · ₹{product.sellingPricePerLitre}</option>)}</select></label><label className="field"><span>New selling price / L</span><input min="0" name="sellingPricePerLitre" required step="0.01" type="number" /></label><label className="field"><span>New cost price / L</span><input min="0" name="costPricePerLitre" required step="0.01" type="number" /></label>
      </SetupForm>
    ...
    <section className="panel panel-pad reveal reveal-3" style={{ marginTop: 16 }}><div className="panel-header"><div><p className="panel-kicker">Active layout</p><h2 className="panel-title">Stations and tank links</h2></div><span className="status-pill healthy">{configuration.stations.length} stations</span></div><table className="data-table"><thead><tr><th>Station</th><th>Product</th><th>Source tank</th><th>Price / L</th><th>Book stock</th></tr></thead><tbody>{configuration.stations.map((station) => { const product = configuration.products.find((item) => item.id === station.productId); const tank = configuration.tanks.find((item) => item.id === station.tankId); return <tr key={station.id}><td><span className="table-title">{station.code}</span><span className="table-subtitle">{station.name}</span></td><td>{product?.name}</td><td>{tankName(station.tankId)}</td><td className="mono">₹{product?.sellingPricePerLitre}</td><td className="mono">{tank?.currentStock} L</td></tr>; })}</tbody></table></section>
  </>;
}
