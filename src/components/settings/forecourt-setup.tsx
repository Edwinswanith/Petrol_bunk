...
export function ForecourtSetup({ configuration }: { configuration: ForecourtConfiguration }) {
  ...
  return <>
    ...
    <div className="three-column reveal reveal-2" aria-busy={saving}>
      <SetupForm title="Add fuel product" description="This outlet currently operates with petrol and diesel." icon={<Fuel color="#0d6b5d" size={20} />} button="Add product" disabled={saving} onSubmit={submit("/api/products")}>
        <label className="field"><span>Code</span><input name="code" placeholder="PETROL" required /></label><label className="field"><span>Name</span><input name="name" placeholder="Petrol" required /></label><label className="field"><span>Customer selling price / L</span><input min="0" name="sellingPricePerLitre" required step="0.01" type="number" /></label><label className="field"><span>Reseller purchase price / L</span><input min="0" name="costPricePerLitre" required step="0.01" type="number" /></label>
      </SetupForm>
      <SetupForm title="Update fuel price" description="The new price applies to future shifts; open and closed shifts retain their snapshots." icon={<Fuel color="#0d6b5d" size={20} />} button="Update price" disabled={saving} onSubmit={updatePrice}>
        <label className="field full"><span>Fuel product</span><select name="productId" required>{configuration.products.filter((product) => product.active).map((product) => <option key={product.id} value={product.id}>{product.name} · ₹{product.sellingPricePerLitre}</option>)}</select></label><label className="field"><span>Customer selling price / L</span><input min="0" name="sellingPricePerLitre" required step="0.01" type="number" /></label><label className="field"><span>Reseller purchase price / L</span><input min="0" name="costPricePerLitre" required step="0.01" type="number" /></label>
      </SetupForm>
      <SetupForm title="Add fuel tank" description="A tank belongs to exactly one fuel product." icon={<Droplets color="#0d6b5d" size={20} />} button="Add tank" disabled={saving} onSubmit={submit("/api/tanks")}>
        <label className="field"><span>Code</span><input name="code" placeholder="PT2" required /></label><label className="field"><span>Name</span><input name="name" placeholder="Petrol Tank 2" required /></label><label className="field full"><span>Fuel product</span><select name="productId" required>{configuration.products.filter((product) => product.active).map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><label className="field"><span>Capacity</span><input min="0.001" name="capacityLitres" required step="0.001" type="number" /></label><label className="field"><span>Opening stock</span><input min="0" name="currentStock" required step="0.001" type="number" /></label>
      </SetupForm>
