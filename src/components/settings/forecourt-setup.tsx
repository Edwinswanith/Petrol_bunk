...
export function ForecourtSetup({ configuration }: { configuration: ForecourtConfiguration }) {
  ...
  const updatePrice = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/products/${productId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sellingPricePerLitre: String(form.get("sellingPricePerLitre")), costPricePerLitre: String(form.get("costPricePerLitre")), marketReferencePrice: String(form.get("marketReferencePrice")) }) });
  ...
  return <>
    ...
    <div className="three-column reveal reveal-2" aria-busy={saving}>
      <SetupForm title="Add fuel product" description="Petrol, diesel, XP95, X100 or any custom grade." icon={<Fuel color="#0d6b5d" size={20} />} button="Add product" disabled={saving} onSubmit={submit("/api/products")}>
        <label className="field"><span>Code</span><input name="code" placeholder="XP95" required /></label><label className="field"><span>Name</span><input name="name" placeholder="XP95" required /></label><label className="field"><span>Selling price / L</span><input min="0" name="sellingPricePerLitre" required step="0.01" type="number" /></label><label className="field"><span>Cost price / L</span><input min="0" name="costPricePerLitre" required step="0.01" type="number" /></label><label className="field full"><span>Market reference / L</span><input min="0" name="marketReferencePrice" required step="0.01" type="number" /></label>
      </SetupForm>
